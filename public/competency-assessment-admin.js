...
function render(){
 ...
 h+='</select></label><button id="caAuto" class="primaryButton">✨ Auto-Generate & Audit 250 Questions</button><button id="caCreate" class="secondary">Create Blank 5-Day Programme</button><button id="caRefresh" class="secondary">Refresh</button></div>';
 ...
 root.querySelector('#caAuto').onclick=async()=>{if(!confirm('Generate 250 high-standard questions for this competency (25 per day), then run two independent answer/ambiguity audits? The result will remain DRAFT for faculty review.'))return;try{setStatus('Generating 5 days × 50 questions and running two audits per day. This may take several minutes…','saving');const r=await call('autoGenerateCompetencyAssessmentProgram')({trackId:selectedTrack});await load();setStatus('✓ '+r.data.message,'success');}catch(e){setStatus(e.message||String(e));}};
...
function renderEditor(t){
 ...
 let h='<div class="caEditor"><div class="caEditorHead"><div><span class="sectionEyebrow">DAY '+t.day+' · '+esc(t.trackTitle)+'</span><h3>'+esc(t.title)+'</h3><p>Status: <b>'+esc(t.status||'draft')+'</b> · '+Number(t.questionCount||0)+' questions · '+Number(t.recommendedQuestionCount||15)+' recommended per student</p></div><span class="practiceBadge">'+(t.isPublished?'PUBLISHED':'DRAFT')+'</span></div>';
